...
export const loadChores = createAction(
  '[Chores] Load Chores',
   props<SetSkipAndTakeAction>()) // represents the call

export const loadChoresSuccess = createAction(
  '[Chores] Load Chores Success',
   props<{ chores: IChoreReadRequest  }>()); // represents the actual data emitted by the observable hence it's not an observable type but it's actual data.
