...
import { loadChoresSuccess, createChoreSuccess, deleteChoreSuccess, loadChoresFailure, loadChoreByIdSuccess, loadChoreByIdFailure } from "./chores.actions";
...
export const choresReducer = createReducer(
  ...
  on(loadChoresSuccess, (state, { chores }) => (
    {chores: chores, error: null, status: 'success'}
    )),
  ...
  on(createChoreSuccess, (state, { chore }) => (
    { ...state, data:[...state.chores.data, chore] }
    )),

  on(deleteChoreSuccess, (state, { id }) => {
    const choreIndex = state.chores.data.findIndex((chore) => 
    chore.id === id
    )

    if(choreIndex !== -1){
      const stateCopy = [...state.chores.data]
      return  { ...state, data: stateCopy.filter((chore) => 
        chore.id !== id
        )}
    }
