...
export class PaginationComponent implements OnDestroy {
  ...
  readonly skip$: Observable<number> = this.paginationData$.pipe(map((x) => x.skip));
  readonly take$: Observable<number> =this.paginationData$.pipe(map((x) => x.take));
  readonly count$: Observable<number> = this.paginationData$.pipe(map((x) => x.count));
  readonly pageCountArr$: Observable<number[]> = this.paginationData$.pipe(
    map((data) => Array.from({ length: Math.ceil(data.count / data.take) }, (_, index) => index ))
  );
  readonly totalNumberOfPages$: Observable<number> = combineLatest([
    this.pageCountArr$,
    this.take$,
  ]).pipe(
    map(([pageCountArr, take]) => pageCountArr.length * take)
  );
  readonly isNextButtonDisabled$ = combineLatest([
    ...
    map(([skip,take, count]) => skip + take > count)
  );
  ...
  public updatePage(page: number) {
    this.take$.pipe(take(1)).subscribe((take) => {
      this.store.dispatch(ChoresActions.loadChores({ skip: page*take, take: take }));
    });
  ...
  public previousPage(){
    combineLatest([
      this.skip$, this.take$
    ]).pipe(take(1)).subscribe(([skip, take]) => {
      this.store.dispatch(ChoresActions.loadChores({skip: (skip - take), take: take}))
    })
